...
官网、客服、公告、Website、Support 等前缀只产生中置信度候选，不会自动排除。管理员可把来源条目强制分类为 `proxy` 或 `notice`；覆盖按节点语义 fingerprint 跨刷新保留。
...
- 网络错误和 HTTP 5xx 最多重试三次，错误文本不得包含来源 URL 或 Token。
