...
Header 与节点名冲突时保留 Header，并把冲突暴露给管理 API。`expire=0` 或 Header 缺失不会静默清除以前已知的到期时间；旧值保留并标为 stale。
...
单位 B、KB、MB、GB、TB、KiB、MiB、GiB、TiB 都按 1024 进制换算。无时区的纯日期按 UTC 当日 `23:59:59` 处理；由于节点名缺少可信时区，默认不允许它直接触发 strict。
...
Profile 自身的 `expires_at` 独立于机场生命周期，到期仍返回 `410`。
