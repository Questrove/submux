...
通用更新引擎已经实现这套次序。在线检查只刷新稳定频道 TUF 元数据；只有操作员确认带所有者绑定和有效期的计划后才下载目标。离线包先作为有大小和摘要上限的本机内容上传，再交给同一 TUF 验证器。签名元数据必须同时声明平台、架构、Runtime 数据库迁移范围、IPC 协议范围、发行说明、磁盘需求以及固定的三个产品组件；包内多出、缺少或摘要不符的条目都会被拒绝。

更新开始前，Runtime 把数据库备份、平台安装器回滚状态、原 Mihomo 状态和原网络设置写入 owner-only 的持久事务。平台安装器的 `PrepareRollback` 只能生成回滚材料，不能停止服务、替换在用程序或迁移数据库；只有持久事务写入成功后才允许进入停机和替换阶段。没有成为持久事务一部分的回滚材料由安装器在启动时清理。中途失败立即恢复；Runtime 在未提交阶段重启时也会先撤销网络接管、停止 Mihomo，再执行相同恢复。完成安装后，平台安装器负责验证程序、服务和本机 IPC，Runtime 再验证数据库、当前配置与 Mihomo 核心，并按原状态重新生成网络预览后恢复接管。配置来源不能提供更新 URL、频道或目标，也不能创建这些 Action。确认计划为一次性凭据；下载、验证或替换失败后必须重新检查并确认，不能重放已消费的计划。

通用引擎不自行猜测 DEB、RPM、MSI 或 PKG 的文件位置和服务命令。各平台交付项必须提供实现固定 `Installer` 合约的安装器，负责程序与数据库的停机替换、验证和回滚；没有安装器时 Snapshot 会如实报告产品更新不可用，界面不能提交安装。
...
发行门禁读取 `release/runtime-release-policy.json`。预览阶段的临时上限必须同时写明批准原因和绝对上限；超过绝对上限仍会失败。例外不改变上表的稳定版目标，进入稳定支持前仍要完成优化或重新作出明确的发行决定。
...
当前固定版本的 tag、提交、六个平台资产摘要、GPL 地址和源码归档摘要记录在 `release/mihomo-v1.19.29.json`。Release 工作流重新下载并逐项验算这些文件；完整离线包同时装入 provenance、GPL 文本和源码归档。
