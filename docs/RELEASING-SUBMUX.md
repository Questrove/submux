...
控制面发行工作流还会运行 Gitleaks、重新执行 Go 检查、构建六个平台资产、生成 SBOM 和 `checksums.txt`，并为全部下载文件生成 GitHub Artifact Attestation。
...
```sh
git tag -a submux-v2.0.1 -m "submux v2.0.1"
git push origin submux-v2.0.1
```
...
- `SBOM.spdx.json` 和 `checksums.txt`。
...
```sh
gh release view submux-v2.0.1
gh release download submux-v2.0.1 --pattern 'checksums.txt' --pattern 'submux-linux-amd64'
sha256sum --check --ignore-missing checksums.txt
gh attestation verify ./submux-linux-amd64 --repo Questrove/submux
```

最后确认 `https://github.com/Questrove/submux/releases/latest` 指向新的控制面 Release，并用稳定通道安装器验证下载、版本匹配和健康检查。
