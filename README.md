...
当前正式版本为 [`v0.3.0`](https://github.com/Questrove/submux/releases/tag/v0.3.0)。安装器固定准确版本并验证 Release 的 SHA-256 清单：

```sh
curl -fsSL https://raw.githubusercontent.com/Questrove/submux/main/scripts/install.sh | bash -s -- --version v0.3.0
```

安装 systemd 服务：

```sh
curl -fsSL https://raw.githubusercontent.com/Questrove/submux/main/scripts/install.sh | bash -s -- --version v0.3.0 --service
```
...
`v0.2.0` 的 Agent 使用旧的 root/Administrator 模型，不能直接原地升级到 `v0.3.0`。升级前请按 [Agent 迁移步骤](docs/AGENT.md#从-v020-迁移) 停止并卸载旧服务，再以普通用户安装和配对新 Agent。
